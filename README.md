...
```mermaid
flowchart LR
  IN[input] --> P[optout<br/>analyze + score]
  P --> OUT[report]
```
...
`optout` is interoperable with every popular way of using AI:

- **MCP server** — `optout mcp` (Claude Desktop, Cursor, Cognis.Studio, [uncensored-fleet](https://github.com/cognis-digital/uncensored-fleet))
- **OpenAI-compatible / JSON** — pipe `optout scan . --format json` into any agent or LLM
