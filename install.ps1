# Comprehensive installer for cognis-digital/optout (Windows PowerShell).
# Tries: pipx -> uv -> pip (git+https) -> from source.
# optout is source-available and not on PyPI; all paths install from GitHub.
$ErrorActionPreference = "Stop"
$Repo = "optout"
$Url  = "git+https://github.com/cognis-digital/optout.git"
$Git  = "https://github.com/cognis-digital/optout.git"
...
if (Have pipx) {
  ...
  pipx install $Url; if ($LASTEXITCODE -eq 0) { Say "Done. Run: optout"; exit 0 }
}
if (Have uv) {
  Say "Installing with uv..."
  uv tool install $Url; if ($LASTEXITCODE -eq 0) { Say "Done. Run: optout"; exit 0 }
}
if (Have pip) {
  Say "Installing with pip (user site)..."
  pip install --user $Url; if ($LASTEXITCODE -eq 0) { Say "Done. Run: optout"; exit 0 }
}
