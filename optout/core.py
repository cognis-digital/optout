...
TOOL_NAME = "OPTOUT"
TOOL_VERSION = "0.1.0"
RULES = [{'id': 'OO-OVERDUE-001', 'severity': 'medium', 'weight': 2.0, 'title': 'BROKER_OVERDUE', 'when': {'days_since_request': '^([4-9]\\d|\\d{3,})'}, 'remediation': 'File CCPA/GDPR complaint with regulator; escalate.'}, {'id': 'OO-REAPPEAR-001', 'severity': 'high', 'weight': 2.5, 'title': 'DATA_REAPPEARED', 'when': {'reappeared': True}, 'remediation': 'Broker non-compliance — re-file + complaint.'}]
        ...
        for rule in RULES:
            if _match(item, rule):
                result.add(Finding(
                    ...
                    location=str(target),
                    remediation=rule.get("remediation",""), category="optout",
                    metadata=item,
                ))
